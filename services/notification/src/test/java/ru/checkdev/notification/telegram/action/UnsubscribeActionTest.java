...
class UnsubscribeActionTest {
    ...
    void handle() {
        ...
        String out = "Для отписки введите данные в формате " + SL
    ...
    void callbackOk() {
        ...
        when(tgAuthCallWebClint.doPost(anyString(), any()))
                .thenReturn(Mono.just(object));
        when(categoryService.categoryExists(any())).thenReturn(true);
        when(subscribeCategoryService.delete(any())).thenReturn(new SubscribeCategory());
        SendMessage result = (SendMessage) action.callback(message);
        assertThat(result.getText())
                .isEqualTo(String.format("email: %s успешно отписан от категории: %s", email, categoryId));
    }
}